...
final class ClubController extends AbstractController
{
    ...
    {
        $validSortByFields = ['nom', 'id'];
        $sortBy = $request->query->get('sort', 'nom');
        $order = strtoupper($request->query->get('order', 'ASC'));
        
        // Validation des paramètres de tri
        $sortBy = in_array($sortBy, $validSortByFields, true) ? $sortBy : 'nom';
        $order = in_array($order, ['ASC', 'DESC'], true) ? $order : 'ASC';
        
        // Création de la requête avec tri et pagination
...
#[Route('/club/pdf', name:'app_club_pdf')]
public function generatePdf(Pdf $knpSnappyPdf, Environment $twig): Response
{
    ...
    $html = $twig->render('club/downloadClubsPDF.html.twig', [
        'clubs' => $clubs,
    ]);
